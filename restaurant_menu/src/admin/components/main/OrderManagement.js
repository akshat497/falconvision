...
import {  FaCheck, FaCheckCircle, FaCheckDouble, FaRegCheckCircle, FaRupeeSign, FaTicketAlt } from 'react-icons/fa';
      ...
      },
    {
      ...
      cell:(row)=>(
       <div>
        ...
        /> */}
        <span><b>{row.Orders[0]?.isActive?<div className='badge bg-info'>New </div>:row.Orders[0]?.isRejected?<div className='badge bg-danger'>Rejected </div>:row.Orders[0]?.isCompleted?<div className='badge bg-success'>Completed </div>:""}</b></span>
       </div>
      )
    },
    
    {
      ...
      cell: (row) => (
        ...
        <div className='d-flex  ' style={{marginLeft:"-30%"}}>
            ...
            <>
            {row.Orders[0]?.isActive?<button className="btn btn-sm btn-success" disabled={row.Orders[0]?.isAccepted===true} onClick={()=>{sendNotifucation(row)}}>{row.Orders[0]?.isAccepted===true?<FaCheckCircle size={30} color="white" />:"Accept"}</button>:''}     
              
              <button onDoubleClick={() => rejectOrder(row)} className={row.Orders[0]?.isActive?'btn btn-sm btn-danger mx-2 ':'btn sm btn-danger mx-2 d-none'}>Reject </button>
              <button onClick={() => completedOrder(row)} className={row.Orders[0]?.isActive?'btn btn-info btn-sm  mr-3':'btn btn-info mr-3 d-none'}>
                 complete
              </button>
